...
export const STAGE075_ANCHOR_REVIEW_BUNDLES: readonly Stage075AnchorReviewBundle[] = [
  {
    ...
    slots: [
      ...
      styleSlot(
        ...
        '앞서 승인된 GIR-SURFACE-30 references를 style parent로 사용한다. canonical Day 1 geography를 고정하지 않는 dawn environment style vignette에서 낮은 지형·암석·식생을 큰 shape/value mass로 단순화한다. WORLD-CAMP-DAWN-A, shelter footprint, route, landmark를 이 이미지로 확정하지 않는다. cinematic bokeh/flare/HDR가 아니라 value/occlusion/perspective로 깊이를 만들고 개별 잎·잔돌·표면 grain을 사진 밀도로 채우지 않는다.',
        [
          ...
          'cool dawn ambient plus local warm contribution without blockbuster grading',
        ],
      ...
      styleSlot(
        ...
        '앞서 승인된 GIR-SURFACE-30 references를 style parent로 사용한다. 자연광 아래 rock, earth, low-specificity garment material을 라벨 표본판이나 macro product shot이 아니라 실제 게임 거리의 큰 면·roughness group·fold/weight로 구분한다. 미세 균열·grain·fiber·stitch를 사진 밀도로 누적하지 않는다.',
        [
          ...
          'all materials share restrained illustrative texture density',
        ],
      ...
      styleSlot(
        ...
        '같은 anonymous style-test moment/source에서 Landscape와 Portrait를 한 쌍으로 파생한다. 먼저 같은 high-resolution source의 crop/zoom으로 두 framing을 해결하고, crop이 action/identity/safe-region을 보존하지 못할 때만 같은 source를 사용한 controlled locked-keyframe variation/outpaint를 허용한다. 두 unrelated text-to-image generation은 금지한다. 인체·손·도구 비율, light direction, color grade와 GIR-SURFACE-30 surface/detail density를 바꾸지 않는다.',
        [
