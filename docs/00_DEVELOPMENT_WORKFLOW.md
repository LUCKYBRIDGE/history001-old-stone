...
- 상황 속 위협과 가치 충돌을 느끼고
- 정답 없는 판단을 하며
- 그 선택의 흔적이 사람·몸·시간·공간·다른 관점에 남는
...
**최종 Hunt 전체를 먼저 만들지 않고, 1인칭 몸·인물·시점 전환이 브라우저에서 실제로 성립하는지 검증한다.**
...
6. R의 도구 전달
7. H1과 출발
8. 짧은 Perspective Bridge prototype
...
기능·Embodied·Relationship Gate 이후 진행한다.
...
먼저 **몸·시야·인물·관점 전환이라는 새 체험 기반**을 작은 골격으로 검증한 뒤 전체 Hunt를 다시 구현한다.
